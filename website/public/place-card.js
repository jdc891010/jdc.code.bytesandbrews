...
function initializePlaceCard() {
  ...
  // Set up spot card click handlers if they exist on the page
  const spotCards = document.querySelectorAll('.spot-card');
  if (spotCards.length > 0) {
    setupSpotCardListeners();
  }
  
  // Expose the openSpotDetail function globally
...
function populateComments(comments) {
  ...
  comments.forEach(comment => {
    ...
    if (!sentiment && comment.rating) {
      ...
      else if (comment.rating <= 2) sentiment = 'negative';
      else sentiment = 'neutral';
    } else if (!sentiment && comment.metrics && comment.metrics.vibe) {
      if (comment.metrics.vibe >= 4) sentiment = 'positive';
      else if (comment.metrics.vibe <= 2) sentiment = 'negative';
      else sentiment = 'neutral';
    } else if (!sentiment) {
      sentiment = 'neutral';
    }
    ...
    groupedComments[sentiment].push(comment);
  });
  
  // Create comments sorted by sentiment
  Object.entries(groupedComments).forEach(([sentiment, list]) => {
    ...
    list.forEach(comment => {
      ...
      const tribe = comment.tribe || 'Digital Nomad';
      
      // Get rating
      let ratingHTML = '';
      const rating = comment.rating || (comment.metrics && comment.metrics.vibe);
      if (rating) {
      ...
      // Get date
      let dateText = '';
      if (comment.timestamp || comment.date) {
        const date = new Date(comment.timestamp || comment.date);
        if (!isNaN(date)) {
          dateText = date.toLocaleDateString();
        } else {
          dateText = comment.timestamp || comment.date;
        }
      }
      ...
      commentDiv.innerHTML = `
        <div class="comment-header">
          ...
          <span class="sentiment-badge ${sentiment}">${sentiment.charAt(0).toUpperCase() + sentiment.slice(1)}</span>
        </div>
        ${ratingHTML}
